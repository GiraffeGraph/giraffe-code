...
export function InputBox({
  ...
  const helpMatches = matches.slice(0, 6);

  return (
    <Box flexDirection="column" paddingLeft={1} paddingTop={1}>
      <Text bold color="yellow">🦒 GIRAFFE CODE — command input</Text>
      <Text dimColor>Mode: {modeLabel(mode, delegateAgent)} • workers stay external</Text>
      ...
      {isSlash ? (
        ...
        <Box marginTop={1} flexDirection="column">
          ...
          <Text dimColor>
            Try: /resume   /delegate codex fix tests   /mode chat
          </Text>
        </Box>
      )}
    </Box>
